...
def gerar_pdf(dados: dict) -> str:
    ...
    try:
        ...
        c.drawImage(_tmp, 1.5*cm, h - 3.1*cm, width=logo_w, height=logo_h,
                    mask="auto")
    ...
    txt("DIRETORIA DE ENSINO REGIÃO DE MAUÁ",
        txt_x, h - 1.65*cm, tamanho=9)
    txt("ESCOLA ESTADUAL EE JOÃO PAULO II",
    ...
    # Então o topo do texto está em cy + 0.247cm e a base em cy.
    # Queremos que o quadrado fique centrado nessa altura total:
    # box_y = baseline - descida - (box - altura_total) / 2
    # Simplificado: box_y = cy - 0.07*cm  (alinha base do box com base do texto)
...
def enviar_whatsapp(numero: str, caminho_pdf: str, aluno: str) -> bool:
    try:
        ...
        mensagem = f"Olá Mãe/Pai, estamos enviando está mensagem para alertar que, seu filho(a) {aluno} recebeu uma ocorrência hoje, em breve iremos te enviar o PDF da ocorrência para que você possa estar ciente, e necessitamos que o quanto antes você venha para a escola assinar a ocorrência."
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(textos, text="DIRETORIA DE ENSINO REGIÃO DE MAUÁ",
                     font=ctk.CTkFont(size=10, weight="bold"),
                     text_color="#1A1A1A", anchor="w").pack(anchor="w")
        ctk.CTkLabel(textos, text="ESCOLA ESTADUAL EE JOÃO PAULO II",
